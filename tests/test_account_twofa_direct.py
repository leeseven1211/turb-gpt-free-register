...
class ProtocolDirectTwofaTests(unittest.TestCase):
    ...
    def _run_worker(self, *, account, fallback_enabled, protocol_side_effect=None, existing_action=None):
        route = self._route()
        ...
        return result, route, db_mocks, append_event, setup_protocol_mock, browser_session, run_browser, action
    ...
    def test_existing_at_uses_protocol_without_opening_roxy(self):
        ...
        result, route, db_mocks, _events, setup_protocol, browser_session, run_browser, _action = self._run_worker(
            account=account,
    ...
    def test_protocol_failure_with_fallback_enters_existing_roxy_path(self):
        ...
        with patch.object(codex_retry_service, "_build_roxy_account_setup", return_value=action) as build_action:
            result, route, _db_mocks, _events, setup_protocol, _browser_session, run_browser, _action = self._run_worker(
                account=account,
                fallback_enabled=True,
