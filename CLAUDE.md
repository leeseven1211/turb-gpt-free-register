...
- `core.registration.dispatcher.run_registration()` switches on `config.roxybrowser.REGISTRATION_DRIVER`: `roxy` (RoxyBrowser + Selenium, primary) or `protocol` (curl_cffi + Sentinel PoW, helper/fallback). `main.run_registration()` remains a CLI compatibility façade.
...
- `core/compat_export.py` — remaining generic/domain/Codex compatibility artifacts are generated by a debounced background task rather than on the write path. Account, registration-job, Outlook, and iCloud pool state stays in PostgreSQL.
...
- The repo root holds live runtime data under `codex_accounts/`, `accounts/`, `注册日志/`, `.env`, `logs/`, and `run/`. Account, registration-job, Outlook, and iCloud pool data is in PostgreSQL; never commit or overwrite credentials.
