...
PostgreSQL 是唯一事实来源；这里只生成仍有外部消费者的通用 API、域名邮箱和
Codex 兼容输出，不参与业务写入。账号、注册任务和邮箱池的根目录快照已退休，
不再由此模块生成。历史上这些快照是在每次写入的主路径上同步生成的：改一个账号
备注要重写大体量 JSON、viewer 和 token 文本，接口要等这几百毫秒才返回。
