...
| --- | --- | --- | --- |
| 0 | 冻结基线与隔离环境 | 无 | 待执行 |
| 1 | 查活路由壳，先只接现有协议 | 无 | 待执行 |
| 2 | 新增 Roxy 旧 AT 浏览器 probe | 默认不启用 | 待执行 |
| 3 | 接入 GitHub `protocol_v2` 查活 | 默认不启用 | 待执行 |
| 4 | 查活小流量放行与用户配置 | 只影响显式选择的任务 | 待执行 |
| 5 | 新协议密码登录 | 浏览器仍默认 | 待执行 |
| 6 | TOTP 与邮箱 OTP challenge | 浏览器仍默认 | 待执行 |
...
现在只执行下面四项，完成前不写 `protocol_v2` 业务代码：

1. [ ] 完成阶段 0 的隔离 worktree、测试库和基线证据。
2. [ ] 完成阶段 1 的 live-check router 设计评审。
3. [ ] 明确 Roxy 普通查活的受控 AT probe 技术方式并做最小诊断实验。
4. [ ] 阶段 2 浏览器 probe 通过后，再开始移植 GitHub `protocol_v2` 查活。
