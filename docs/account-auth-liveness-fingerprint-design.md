...
6. 同一“刷新 AT”任务内允许受控改走一次邮箱 OTP；使用相同账号级 Protocol 设备画像，但创建新的会话层 ID 和 Cookie jar。
7. 邮箱 OTP 后如果远端仍返回 MFA challenge，继续使用保存的 TOTP 完成验证。

密码错误后的邮箱兜底默认启用，因为用户已经明确发起“刷新 AT”，邮箱认证属于同一授权范围。它不是静默掩盖：任务事件和账号最近认证结果必须记录“密码被拒绝、邮箱兜底是否成功”。
