...
## 12. 生产切换结果（2026-08-25）

- 停机备份已验证可读取；迁移前五张旧表逐行内容与演练库一致；
- 迁移完成后：656/656 条注册运行、2508/2508 条账号运行、9779/9779 条账号事件完成映射；
- 孤儿运行和孤儿事件均为 0，待邮箱验证尝试最终归零；
- 账号 338、347、353、354、355 均已取得 Token、启用 2FA、确认 free 套餐，目标状态为
  `account_available`；
- 独立开发库切换当时的历史验证为全量 485 项；阶段 0 基线（含路由契约）为 502 项，当前仓库基线（含阶段 1 回归测试）为 504 项，正式 WebUI 浏览器控制台无错误。
